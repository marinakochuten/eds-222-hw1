...
-   In this setting, the population is the pollution condition across all of Lahore over the given time period, November 2018 to November 2019.

2.  What are the *samples* in this setting? Please be precise.
    -   The samples are the two datasets, one sample is the crowdsourced data and the other sample is the government data. Since the data are collected with different methodologies, they are separate samples. Both samples represent daily observations taken from November 2018 to November 2019.
...
-   The crowd-sourced data comes from monitors that were voluntarily placed in individuals households. Self-selection bias could come into play here, as people who would choose to monitor and upload pollution data are going to represent a particular slice of the population, and may not be representative of pollution as a whole. People who may be uploading data are probably more educated and affluent. It is also possible that people uploading data are concerned about their air quality near their home, because they live in an area with poorer air quality.
...
-   Yes, it makes sense that the mean PM 2.5 concentration would be much lower in the government data, as they only placed monitors in places known to have clean air. On the other hand, bias from the crowdsourced data would result in a higher mean PM 2.5 concentration, as the observations are coming from homes where people are most likely concerned about poor air quality nearby.
...
# check dataset dimensions for random sample, rows should = 1000
dim(cs_govt_sample)
```
...
# check dataset dimensions for stratified random sample, rows should = 1000
dim(cs_govt_sample)
```
